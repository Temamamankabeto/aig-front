...
function lineUnit(line?: PurchaseOrderItemRow) {
  ...
  return inv?.base_unit ?? line?.unit ?? "pcs";
}
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Purchaser creates requests, Food Controller validates, Manager approves, and Stock Keeper receives."
        icon={ClipboardList}
      />
      <Card>
        <CardHeader>
...
function PurchaseOrderForm({ onDone }: { onDone?: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: (payload: PurchaseOrderPayload) =>
      procurementService.createPurchaseOrder(payload, "purchaser"),
    onSuccess: () => {
    ...
    const cleanLines = lines
      ...
      .map((line) => {
        ...
        return {
          ...
          base_unit: inv?.base_unit ?? inv?.unit ?? "pcs",
...
function PurchaseOrdersTable({
  ...
  const submit = useMutation({
    mutationFn: (id: number) =>
      procurementService.submitPurchaseOrder(id, "purchaser"),
    onSuccess: () => {
