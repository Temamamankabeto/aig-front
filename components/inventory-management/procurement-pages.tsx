...
import { can, getStoredRoles, inventoryPermissions, purchasePermissions } from "@/lib/auth/permissions";
...
function normalizedStoredRoles() {
  return getStoredRoles().map((role) => String(role ?? "").trim().toLowerCase().replace(/&/g, "and").replace(/[\s-]+/g, "_"));
}
function isStoreKeeperRole() {
  return normalizedStoredRoles().some((role) => ["store_keeper", "stock_keeper", "warehouse", "warehouse_keeper"].includes(role));
}
function purchaseRequestScope(): "purchaser" | "stock-keeper" {
  return isStoreKeeperRole() ? "stock-keeper" : "purchaser";
}
function canCreatePurchaseOrder() { return can(purchasePermissions.ordersCreate) || can(purchasePermissions.requestsCreate) || isStoreKeeperRole(); }
...
export function statusBadge(status?: string | null) {
  ...
  if (["food_validated", "approved", "completed"].includes(normalized)) return <Badge variant="default">{normalized.replaceAll("_", " ")}</Badge>;
...
function PurchaseOrderForm({ onDone }: { onDone?: () => void }) {
  ...
  const inventory = useInventoryItemsQuery({ per_page: 100, status: "active" }, purchaseRequestScope());
  ...
  const mutation = useMutation({ mutationFn: (payload: PurchaseOrderPayload) => procurementService.createPurchaseOrder(payload, purchaseRequestScope()), onSuccess: () => { toast.success("Purchase request created"); qc.invalidateQueries({ queryKey: ["procurement", "purchase-orders"] }); onDone?.(); }, onError: (error) => toast.error(extractError(error, "Failed to create purchase request")) });
...
function PurchaseOrdersTable({ rows, loading }: { rows: PurchaseOrderRow[]; loading?: boolean }) {
  const qc = useQueryClient();
  const submit = useMutation({ mutationFn: (id: number) => procurementService.submitPurchaseOrder(id), onSuccess: () => { toast.success("Purchase order submitted"); qc.invalidateQueries({ queryKey: ["procurement", "purchase-orders"] }); }, onError: (error) => toast.error(extractError(error, "Failed to submit")) });
  const approve = useMutation({ mutationFn: (id: number) => procurementService.approvePurchaseOrder(id, "manager"), onSuccess: () => { toast.success("Purchase order approved and ready for receiving"); qc.invalidateQueries({ queryKey: ["procurement", "purchase-orders"] }); }, onError: (error) => toast.error(extractError(error, "Failed to approve")) });
